...
public class AttemptsController : ControllerBase
{
    ...
    public AttemptsController(IAttemptRepository attemptRepository, IExamRepository examRepository, ISessionRepository sessionRepository)
    {
    ...
    {
        if (Request.Headers.TryGetValue("X-Session-Id", out StringValues header) && Guid.TryParse(header, out var id))
        {
            sessionId = id;
            return true;
        }
    ...
    public async Task<IActionResult> Start([FromBody] StartAttemptRequest request)
    {
        ...
        var existing = await _attemptRepository.GetLatestUnfinishedAsync(sessionId, request.ExamId);
        if (existing != null)
        ...
        int singleToTake = exam.SingleChoiceToShow > 0 ? Math.Min(exam.SingleChoiceToShow, singleQuestions.Count) : singleQuestions.Count;
        int multipleToTake = exam.MultipleChoiceToShow > 0 ? Math.Min(exam.MultipleChoiceToShow, multipleQuestions.Count) : multipleQuestions.Count;
        int textToTake = exam.TextInputToShow > 0 ? Math.Min(exam.TextInputToShow, textQuestions.Count) : textQuestions.Count;
    ...
    public async Task<IActionResult> Finish(Guid id, [FromBody] FinishAttemptRequest request)
    {
        ...
        var storedAnswers = request.Answers.Select(a =>
        {
            object? answerValue = a.Answer switch
            {
                ...
                JsonElement json when json.ValueKind == JsonValueKind.Array => json.EnumerateArray().Select(e => e.GetString()).ToList(),
    ...
    public async Task<IActionResult> GetResult(Guid id)
    {
        ...
        {
            var stored = attempt.Answers.FirstOrDefault(a => a.QuestionId == q.Id);
